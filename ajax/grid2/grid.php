...
if($action == 'load')
{
	$page = $_POST['page'] ?? 1; // get the requested page
	$limit = $_POST['rows'] ?? 5; // get how many rows we want to have into the grid
	$sidx = $_POST['sidx'] ?? 'product_id'; // get index row - i.e. user click to sort
	$sord = $_POST['sord'] ?? 'asc'; // get the direction
